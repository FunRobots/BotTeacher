...
class SchemeProcessor:
    '''
    Allows to process BotTeacher scenario scheme
    '''
    ...
    def _say_phrase(self, phrase_to_say: str) -> str or None:
        '''
        say phrase to the bot and return text answer or None
        '''
    ...
    def _wait_before_transition(self, variants_dict: dict) -> str or None:
        '''
        switch to one of variants depending of pause duration
        '''

        wait_keys = [int(i) for i in variants_dict.keys()] #all pause duration variants (keys of variants dictionary)
        wait_keys.sort() #sort variants
        
        case_key_index = 0 #find variant index
        while case_key_index < len(wait_keys) and self.pause > wait_keys[case_key_index]:
            case_key_index += 1

        case = variants_dict[str(wait_keys[case_key_index])] #choose case (action variant) in according to pause)
        
        self.current_intent_name = case.get('goto_next') #read next intent name
        
        phrase_to_say = case.get('say_phrase') #read phrase to send to bot
        ...
        return case.get('return') #if case has 'return' key return it value
    ...
    def process(self):
        ...
        while self.current_intent_name is not None:
            ...
            #request to bot Consultant speech content
            bot_answer = self.bot.request(speech_text)
            if isinstance(bot_answer, dict) and bot_answer.get('intent_name') == self.current_intent_name: #if intent name in bot answer is equal to current intent name
                current_intent = self.scheme['intents'].get(self.current_intent_name) #try extract intent dictionary
                if isinstance(current_intent, dict):
                    variants_dict = current_intent.get('wait_before_transition') #extract pause-depending variants 
                else:
                    variants_dict = None
                    current_intent = dict()

                if variants_dict is not None:
                    #proccess pause-depending variants dictionary
                ...
                else: #if current intent has no field 'wait_before_transition' just communicate with bot
            ...
            else: #if intent name in bot answer is not equal to current intent name inform about it
